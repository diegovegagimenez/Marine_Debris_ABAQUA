...
# Since the main focus of this article is clustering geospatial data, I will keep this section very simple.
# Inside each cluster, I will select the potential targets (BARQUES) and hubs (EFFICIENT PORTS),
# and relocate the staff of the targets in the hubs until the latter reach full capacity. When the whole
# staff of a target is moved, the store can be closed.
...
for c in sorted(dtf["cluster"].unique()):
    ...
    ## hubs and targets
    lst_hubs = dtf_cluster[dtf_cluster["Barca"] == "low"
                           ].sort_values("Kg").to_dict("records")
    lst_targets = dtf_cluster[dtf_cluster["Barca"] == "high"
                              ].sort_values("Kg").to_dict("records")
    ## move targets
